...
class RemoteReplicatingNestedJobParser[J <: JsonJob](codec: JsonCodec[J], hosts: Seq[String], port: Int, priority: Int) extends JsonNestedJobParser(codec) {
  ...
  override def apply(json: Map[String, Any]) = {
    val replicated = json.get("replicated").map { _.asInstanceOf[Boolean] }.getOrElse(false)
