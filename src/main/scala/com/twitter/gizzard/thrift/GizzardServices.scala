...
class GizzardServices[S <: Shard, J <: JsonJob](shardServerPort: Int,
  ...
  val shardThriftServer = TThreadServer("shards", shardServerPort, idleTimeout.inMillis.toInt, gizzardThreadPool, shardProcessor)

  val jobServer = new JobManagerService(scheduler)
  val jobProcessor = new JobManager.Processor(jobServer)
  val jobThriftServer = TThreadServer("jobs", jobServerPort, idleTimeout.inMillis.toInt, gizzardThreadPool, jobProcessor)
