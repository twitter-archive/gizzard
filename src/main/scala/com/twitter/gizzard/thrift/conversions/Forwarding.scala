...
object Forwarding {
  class RichShardingForwarding(forwarding: nameserver.Forwarding) {
    def toThrift = new thrift.Forwarding(forwarding.tableId, forwarding.baseId, forwarding.shardId)
  }
  ...
  class RichThriftForwarding(forwarding: thrift.Forwarding) {
    def fromThrift = new nameserver.Forwarding(forwarding.table_id, forwarding.base_id, forwarding.shard_id)
  }
