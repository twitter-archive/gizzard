...
object ChildInfo {
  class RichShardingChildInfo(childInfo: sharding.ChildInfo) {
    def toThrift = new thrift.ChildInfo(childInfo.shardId, childInfo.position, childInfo.weight)
  }
  ...
  class RichThriftChildInfo(childInfo: thrift.ChildInfo) {
    def fromThrift = new sharding.ChildInfo(childInfo.shard_id, childInfo.position, childInfo.weight)
  }
