...
class ManagerService[S <: shards.Shard, J <: JsonJob](nameServer: NameServer[S], copier: CopyJobFactory[S], scheduler: PrioritizingJobScheduler[J], copyScheduler: JobScheduler[J]) extends Manager.Iface {
  ...
  def rebuild_schema() =
    wrapEx(nameServer.rebuildSchema())
  ...
  def copy_shard(sourceId: ShardId, destinationId: ShardId) =
    wrapEx(copyScheduler.put(copier(sourceId.fromThrift, destinationId.fromThrift).asInstanceOf[J]))
