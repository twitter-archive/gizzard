...
class QueueCollection(queueFolder: String, defaultConfig: QueueConfig, private var queueConfigs: Map[String, QueueConfig]) {
  ...
  def add(key: String, item: Array[Byte], expiry: Int): Boolean = {
    ...
    queue(key) match {
      ...
      case Some(q) =>
        ...
        val result = q.add(item, Some(Time(normalizedExpiry.millis)))
   ...
   */
  def remove(key: String, timeout: Duration, transaction: Boolean, peek: Boolean)(f: Option[QItem] => Unit): Unit = {
    queue(key) match {
      case None =>
        queueMisses.incr
        f(None)
      case Some(q) =>
        if (peek) {
          ...
          q.removeReact(if (timeout == 0) Some(Time(timeout)) else Some(Time.now + timeout), transaction) {
            case None =>
              queueMisses.incr
              f(None)
            case Some(item) =>
  ...
  def receive(key: String): Option[Array[Byte]] = {
    ...
    remove(key, 0.seconds, false, false) {
      case None =>
        rv = None
        latch.countDown
      case Some(v) =>
  ...
  def stats(key: String): Array[(String, String)] = queue(key) match {
    ...
    case Some(q) =>
      Array.concat(q.dumpStats(), fanout_queues.get(key).map { qset => ("children", qset.mkString(",")) }.toList.toArray)
  }
